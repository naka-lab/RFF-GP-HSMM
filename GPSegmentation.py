...
class GPSegmentation():
    def __init__(self, dim, nclass, min_max_ave_len=(15,20,30), num_rff_features=-1, sigma_rff=0.1, alpha_rff=1.0, beta_rff=10.0):
        ...
        self.segmlen = 3
        self.gps = [ GPRFF.GPMD(dim, num_rff_features, sigma_rff, alpha_rff, beta_rff) for i in range(self.numclass) ]
        ...
        self.min_len = min_max_ave_len[0]
        self.ave_len = min_max_ave_len[1]
        self.max_len = min_max_ave_len[2]
    ...
    def calc_vitervi_path(self, d ):
        T = len(d)
        log_a = np.zeros( (len(d), self.max_len, self.numclass) )  - 9999999999999999999 
    ...
    def forward_filtering(self, d ):
        ...
        # 全時刻の出力確率をあらかじめ計算
        emission_prob_all = self.calc_emission_logprob_all( d )
        forward_prob = calc_forward_probability( emission_prob_all, self.trans_prob, self.trans_prob_bos, self.trans_prob_eos, len(d), self.min_len, 1, self.max_len, self.numclass )
        return forward_prob
    ...
    def update(self, learning_phase=True ):
        ...
        """
        for c in range(self.numclass):
            x = np.linspace(0, self.MAX_LEN, 100)
            self.gps[c].plot(x)
            plt.figure()
        plt.show()
        """
